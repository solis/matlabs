function visualize(x_min, x_max, X, T, analytical_grid, numerical_grid)
    ...
    while(w ~= 1)
        for t = 1:length(T)
            ...
            xlim([x_min x_max]);
            ylim([u_min u_max]);
            title(sprintf('Анимация аналитического решения: t = %.1f', T(t)));
            xlabel ('x');
            ylabel ('u(x,t)');
            pause(0.05);
        end
